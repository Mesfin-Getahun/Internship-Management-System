...
const UilSidebar = ({ activeTab }) => {
  const menuItems = [
    ...
    { id: 'approvals', label: 'Org Approvals', icon: 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z' },
    { id: 'monitoring', label: 'Internship Monitoring', icon: 'M15 12a3 3 0 11-6 0 3 3 0 016 0z' },
    { id: 'reports', label: 'Fulfillment Reports', icon: 'M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z' },
  ...
  return (
    <aside className="w-64 bg-slate-900 border-r border-slate-800 flex flex-col h-screen fixed left-0 top-0 pt-20 z-40 transition-colors">
      <div className="px-6 py-4 border-b border-slate-800">
        <h2 className="text-white font-black text-xs uppercase tracking-[0.2em] opacity-50">University UIL</h2>
      </div>
      <nav className="flex-grow px-4 mt-4 space-y-1 overflow-y-auto custom-scrollbar">
        {menuItems.map(item => (
          <Link
            key={item.id}
            to={`/dashboard/uil-officer/${item.id}`}
