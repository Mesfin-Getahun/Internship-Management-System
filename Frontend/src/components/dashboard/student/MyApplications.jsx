    ...
    const fetchApplication = async () => {
      try {
        const res = await axios.get(
          "http://localhost:5000/api/student/myApplication",
          {
            headers: {
    ...
    <div className="animate-fade-in space-y-6">
      <header>
        ...
        <p className="text-slate-500 text-sm mt-1">
          Track your progress with partner organizations.
        </p>
      </header>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800">
                <th className="p-5 text-xs font-black uppercase text-slate-400 tracking-widest whitespace-nowrap">
                  Role / Organization
                </th>
                <th className="p-5 text-xs font-black uppercase text-slate-400 tracking-widest whitespace-nowrap">
                  Applied Date
                </th>
                <th className="p-5 text-xs font-black uppercase text-slate-400 tracking-widest text-center whitespace-nowrap">
                  Status
                </th>
                <th className="p-5 text-xs font-black uppercase text-slate-400 tracking-widest text-right whitespace-nowrap">
                  Action
                </th>
              </tr>
            </thead>
      ...
      <div className="p-8 bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-800 rounded-3xl">
        ...
        <ul className="space-y-2">
          {[
            "Students can apply to multiple internships simultaneously.",
            "Once submitted, applications cannot be edited.",
            "If accepted by multiple orgs, you must coordinate with Faculty for placement.",
          ].map((text, i) => (
            <li
              key={i}
              className="flex items-start gap-3 text-xs text-blue-600 dark:text-blue-500 leading-relaxed font-medium"
            >
