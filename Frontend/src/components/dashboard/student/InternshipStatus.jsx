...
const InternshipStatus = () => {
  ...
  //       range: "1-2",
  //       date: "Oct 15, 2023",
  //       preview: "Initial onboarding and platform architecture review.",
  //     },
  //     {
  //       range: "3-4",
  //       date: "Oct 29, 2023",
  //       preview: "Tasked with monitoring script development. Good progress.",
  //     },
  //     {
  //       range: "5-6",
  //       date: "Nov 12, 2023",
  //       preview: "Presented script to the team. Received positive feedback.",
  //     },
  //   ],
  // };
    ...
    const fetchInternship = async () => {
      try {
        const res = await axios.get(
          "http://localhost:5000/api/student/myInternship",
          {
            headers: {
    ...
    <div className="animate-fade-in space-y-8 pb-12">
      ...
      <div className="bg-white dark:bg-slate-800/50 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm p-8">
        ...
        <div className="mt-8 pt-8 border-t border-slate-100 dark:border-slate-700/50 grid grid-cols-1 md:grid-cols-3 gap-8">
          <InfoCard
            icon={Building}
            label="Organization"
            value={internshipData.company_name}
          />
          <InfoCard
            icon={User}
            label="Company Supervisor"
            value={internshipData.company_mentor_name}
            subValue={internshipData.company_mentor_email}
          />
          <InfoCard
            icon={User}
            label="Assigned Mentor"
            value={internshipData.faculty_mentor_name}
            subValue={internshipData.faculty_mentor_email}
          />
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800/50 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-slate-100 dark:border-slate-700/50 flex justify-between items-center">
          <h4 className="font-bold text-slate-700 dark:text-white uppercase text-sm tracking-wider">
            Supervisor Bi-Weekly Notes
          </h4>
